    ...
    def __call__(self, query, action, focus, target,debug=False):
        ...
        if actionassignments:
            if ('set' not in actionassignments or actionassignments['set'] is False) and action.focus.Class:
                if action.focus.set is not False:
                    actionassignments['set'] = action.focus.set
                elif action.focus.Class.XMLTAG in query.defaultsets:
                    actionassignments['set'] = query.defaultsets[action.focus.Class.XMLTAG]
                else:
                    defaultset = query.doc.defaultset(action.focus.Class)
        ...
        if self.action == 'SUBSTITUTE' and not self.focus and self.form:
            ...
            for action in actions:
                ...
                if action.action in ("ADD","APPEND","PREPEND") or (action.action == "EDIT" and not focusselection):
                    ...
                    if 'set' not in action.assignments and action.focus.Class not in (folia.Description, folia.Comment, folia.AbstractFeature) and not isspanrole:
                    ...
                    required = action.focus.Class.REQUIRED_ATTRIBS
                    allowed = action.focus.Class.OPTIONAL_ATTRIBS
                    if required is None:
                        required = tuple()
                    if allowed is None:
                        allowed = tuple()
                    ...
                    if query.doc.processor and 'processor' not in action.assignments and folia.Attrib.ANNOTATOR in supported:
                        action.assignments['processor'] = query.doc.processor
